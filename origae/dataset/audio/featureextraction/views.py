...
from .forms import AudioFeatureExtractionDatasetForm
from .job import AudioFeatureExtractionDatasetJob
...
def new():
    """
    Returns a form for a new AudioFeatureExtractionDatasetForm
    """
    form = AudioFeatureExtractionDatasetForm()

    # Is there a request to clone a job with ?clone=<job_id>
    fill_form_if_cloned(form)

    return flask.render_template('datasets/audio/featureextraction/new.html', form=form)
...
def create():
    ...
    if not form.validate_on_submit():
        if request_wants_json():
            ...
            return flask.render_template('datasets/audio/featureextraction/new.html', form=form), 400

    job = None
    try:
        job = AudioFeatureExtractionDatasetJob(
...
def show(job, related_jobs=None):
    ...
    """
    return flask.render_template('datasets/audio/featureextraction/show.html', job=job, related_jobs=related_jobs)


def summary(job):
    """
    Return a short HTML summary of an AudioFeatureExtractionDatasetJob
    """
    return flask.render_template('datasets/audio/featureextraction/summary.html',
                                 dataset=job)
