...
from .forms import GenericAudioDatasetForm
from .job import GenericAudioDatasetJob
...
def new():
    ...
    """
    form = GenericAudioDatasetForm()

    # Is there a request to clone a job with ?clone=<job_id>
    fill_form_if_cloned(form)

    return flask.render_template('datasets/audio/generic/new.html', form=form)
...
def create():
    ...
    if not form.validate_on_submit():
        if request_wants_json():
            ...
            return flask.render_template('datasets/audio/generic/new.html', form=form), 400

    job = None
    try:
        job = GenericAudioDatasetJob(
...
def show(job, related_jobs=None):
    ...
    """
    return flask.render_template('datasets/audio/generic/show.html', job=job, related_jobs=related_jobs)
...
def summary(job):
    ...
    """
    return flask.render_template('datasets/audio/generic/summary.html',
                                 dataset=job)
