...
@subclass
class AudioSegmentationDatasetJob(AudioDatasetJob):
    """
    A Job that creates an image dataset for a classification network
    """

    def __init__(self, **kwargs):
        super(AudioSegmentationDatasetJob, self).__init__(**kwargs)
        self.pickver_job_dataset_image_classification = PICKLE_VERSION

        self.labels_file = None

    def __setstate__(self, state):
        super(AudioSegmentationDatasetJob, self).__setstate__(state)
    ...
    def json_dict(self, verbose=False):
        d = super(AudioSegmentationDatasetJob, self).json_dict(verbose)
