...
from origae.dataset import TextClassificationDatasetJob
from origae.inference import TextInferenceJob
...
def new():
    ...
    return flask.render_template('models/text/classification/new.html',
                                 form=form,
...
def show(job, related_jobs=None):
    ...
    return flask.render_template(
        'models/text/classification/show.html',
        job=job,
        framework_ids=[
            fw.get_id()
...
def classify_one():
    ...
    inference_job = TextInferenceJob(
        username=utils.auth.get_username(),
    ...
    if request_wants_json():
        ...
        return flask.render_template('models/text/classification/classify_one.html',
                                     model_job=model_job,
                                     job=inference_job,
                                     image_src=image,
...
def classify_many():
    ...
    if request_wants_json():
        ...
        return flask.render_template('models/text/classification/classify_many.html',
                                     model_job=model_job,
                                     job=inference_job,
                                     paths=paths,
...
def top_n():
    ...
    return flask.render_template('models/text/classification/top_n.html',
                                 model_job=model_job,
                                 job=inference_job,
                                 results=results,
                                 )
...
def get_previous_networks():
    return [(j.id(), j.name()) for j in sorted(
        [j for j in scheduler.jobs.values() if isinstance(j, TextClassificationModelJob)],
        cmp=lambda x, y: cmp(y.id(), x.id())
    )
    ]
