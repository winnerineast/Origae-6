...
from origae.dataset import GenericDatasetJob, GenericTextDatasetJob
...
def get_datasets(extension_id):
    if extension_id:
        ...
        jobs = [j for j in scheduler.jobs.values()
                if (isinstance(j, GenericTextDatasetJob) or isinstance(j, GenericDatasetJob)) and
...
def get_previous_networks():
    return [(j.id(), j.name()) for j in sorted(
        [j for j in scheduler.jobs.values() if isinstance(j, GenericTextModelJob)],
