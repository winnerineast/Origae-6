...
from .forms import GenericAudioModelForm
from .job import GenericAudioModelJob
...
from origae.dataset import GenericDatasetJob, GenericAudioDatasetJob
...
def show(job, related_jobs=None):
    ...
    return flask.render_template(
        'models/audio/generic/show.html',
        job=job,
...
def infer_db():
    ...
    if request_wants_json():
        ...
        return flask.render_template(
            'models/audio/generic/infer_db.html',
            model_job=model_job,
            job=inference_job,
            keys=keys,
            inference_views_html=inference_views_html,
...
def infer_many():
    ...
    if request_wants_json():
        ...
        return flask.render_template(
            'models/audio/generic/infer_many.html',
            model_job=model_job,
            job=inference_job,
            paths=paths,
            inference_views_html=inference_views_html,
...
def get_datasets(extension_id):
    if extension_id:
        ...
        jobs = [j for j in scheduler.jobs.values()
                if (isinstance(j, GenericAudioDatasetJob) or isinstance(j, GenericDatasetJob)) and
...
def get_previous_networks():
    return [(j.id(), j.name()) for j in sorted(
        [j for j in scheduler.jobs.values() if isinstance(j, GenericAudioModelJob)],
