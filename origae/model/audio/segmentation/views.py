...
from .forms import AudioSegmentationModelForm
from .job import AudioSegmentationModelJob
from origae import frameworks
from origae import utils
from origae.config import config_value
from origae.dataset import AudioSegmentationDatasetJob
...
def new():
    ...
    """
    form = AudioSegmentationModelForm()
    ...
    return flask.render_template('models/audio/segmentation/new.html',
                                 form=form,
...
def show(job, related_jobs=None):
    ...
    return flask.render_template(
        'models/audio/segmentation/show.html',
        job=job,
        framework_ids=[
            fw.get_id()
...
def classify_one():
    ...
    if request_wants_json():
        ...
        return flask.render_template('models/audio/segmentation/classify_one.html',
                                     model_job=model_job,
                                     job=inference_job,
                                     image_src=image,
...
def classify_many():
    ...
    if request_wants_json():
        ...
        return flask.render_template('models/audio/segmentation/classify_many.html',
                                     model_job=model_job,
                                     job=inference_job,
                                     paths=paths,
...
def get_datasets():
    return [(j.id(), j.name()) for j in sorted(
        [j for j in scheduler.jobs.values() if isinstance(j, AudioSegmentationDatasetJob) and
...
def get_previous_networks():
    return [(j.id(), j.name()) for j in sorted(
        [j for j in scheduler.jobs.values() if isinstance(j, AudioSegmentationModelJob)],
        cmp=lambda x, y: cmp(y.id(), x.id())
    )
    ]
