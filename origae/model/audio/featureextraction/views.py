...
from .forms import AudioFeatureExtractionModelForm
from .job import AudioFeatureExtractionModelJob
from origae import frameworks
from origae import utils
from origae.config import config_value
from origae.dataset.audio import AudioFeatureExtractionDatasetJob
from origae.inference import AudioInferenceJob
...
def new():
    ...
    """
    form = AudioFeatureExtractionModelForm()
    ...
    return flask.render_template('models/audio/featureextraction/new.html',
                                 form=form,
...
def create():
    ...
    for sweep in sweeps:
        ...
        job = None
        try:
            job = AudioFeatureExtractionModelForm(
...
def show(job, related_jobs=None):
    ...
    return flask.render_template(
        'models/audio/featureextraction/show.html',
        job=job,
        framework_ids=[
            fw.get_id()
...
def classify_one():
    ...
    inference_job = AudioInferenceJob(
        username=utils.auth.get_username(),
    ...
    if request_wants_json():
        ...
        return flask.render_template('models/audio/featureextraction/classify_one.html',
                                     model_job=model_job,
                                     job=inference_job,
                                     image_src=image,
...
def classify_many():
    ...
    if request_wants_json():
        ...
        return flask.render_template('models/audio/featureextraction/classify_many.html',
                                     model_job=model_job,
                                     job=inference_job,
                                     paths=paths,
...
def top_n():
    ...
    return flask.render_template('models/audio/featureextraction/top_n.html',
                                 model_job=model_job,
                                 job=inference_job,
                                 results=results,
                                 )
...
def get_datasets():
    return [(j.id(), j.name()) for j in sorted(
        [j for j in scheduler.jobs.values() if isinstance(j, AudioFeatureExtractionDatasetJob) and
...
def get_previous_networks():
    return [(j.id(), j.name()) for j in sorted(
        [j for j in scheduler.jobs.values() if isinstance(j, AudioFeatureExtractionModelJob)],
        cmp=lambda x, y: cmp(y.id(), x.id())
    )
    ]
